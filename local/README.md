...
| `claude` | Selective shim: jims-team's CoS seat (`--name exec-cos@jims-team*`) boots via `ollama launch claude --model $JIMS_COS_MODEL` (default `kimi-k3:cloud`); everything else passes through to the real binary. NB: Claude Code's `--name` flag hangs the ollama path — the shim strips it; seat identity lives in the tmux session name anyway |

## Ops notes

- **CoS vision support depends on $JIMS_COS_MODEL — `glm-5.3:cloud` rejected image input (HTTP 400, session-poisoning); verify the current model before pasting images with HTTP 400, and a pasted image poisons the session — every later turn replays it and fails). Route screenshots/UI evidence to the claude seats (or codex seats) instead. Recovery recipe after a poisoned CoS turn: `/exit` in the pane → `ollama launch claude --model "$JIMS_COS_MODEL" -- --dangerously-skip-permissions` → paste `~/.openrig/specs/jims-team/agents/chief-of-staff/guidance/role.md` (tmux load-buffer/paste-buffer) → submit → `rig reconcile-session exec-cos@jims-team`.
- **Codex seats bind profile `rig-yolo`** (`codex_config_profile: rig-yolo` on the three verify-pod members; daemon passes `-p rig-yolo` on launch/resume/handover). Profile lives at `~/.codex/rig-yolo.config.toml` (codex 0.156 requires split-file profiles; legacy `[profiles.*]` in config.toml is rejected at preflight): `approval_policy = "never"` + `sandbox_mode = "danger-full-access"`. Personal interactive codex (no profile) is back to default ask/sandbox behavior. Verified: preflight profile-load probe clean; scratch `codex exec --profile rig-yolo` → PROFILE-OK, no prompt.
