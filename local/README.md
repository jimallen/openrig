...
- **Codex is now yolo-by-default machine-wide**: `~/.codex/config.toml` (and the rig-codex copy) sets `approval_policy = "never"` + `sandbox_mode = "danger-full-access"` — openrig's yolo policy only added `-s danger-full-access` at launch (sandbox off), which left codex's default approval prompts firing. Personal interactive codex inherits the same posture; that's deliberate. Verified with a scratch `codex exec` (CFG-OK, no prompt).
